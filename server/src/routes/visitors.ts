...
router.post("/", requireRole("security", "md"), (req, res) => {
...
router.patch("/:id/checkout", requireRole("security", "md"), (req, res) => {
  const updated = storage.updateVisitor(parseInt(req.params.id), { timeOut: new Date() });
