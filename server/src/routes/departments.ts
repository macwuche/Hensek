...
router.post("/", requireRole("md", "hr"), async (req, res) => {
  ...
  const dept = await storage.createDept({
    name,
    slug,
    type: "standard",
    description,
...
router.put("/:id", requireRole("md"), async (req, res) => {
  const { name, description, email, csoEmail, safetyEmail } = req.body;
  const updated = await storage.updateDept(parseInt(req.params.id), { name, description, email, csoEmail, safetyEmail });
...
router.delete("/:id", requireRole("md"), async (req, res) => {
  ...
  const deleted = await storage.deleteDept(parseInt(req.params.id));
